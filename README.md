[![NPM](https://img.shields.io/npm/l/react)](https://github.com/soleralucas/assets/blob/main/LICENSE) 

# Sobre o projeto

Projeto desenvolvido durante uma aula com o objetivo de praticar HTML e CSS por meio da construção de uma página web proposta pelo professor.

O desafio consistiu em reproduzir a interface apresentada em aula utilizando conceitos de estruturação de páginas, posicionamento de elementos, estilização e organização do conteúdo, aplicando exclusivamente HTML e CSS.

A aplicação é composta por três páginas distintas, permitindo exercitar a navegação entre páginas e a construção de layouts estáticos de forma consistente.

O projeto também possui um layout responsivo, permitindo que o conteúdo se adapte ao redimensionamento da janela do navegador sem cortes ou perda de informações.

## Imagens da aplicação

![Página Quem Somos](https://github.com/soleralucas/assets/blob/main/projeto-inteling%C3%AAncia-artificial/Captura%20de%20tela%202026-06-04%20235243.png)

![Página Nossos serviços](https://github.com/soleralucas/assets/blob/main/projeto-inteling%C3%AAncia-artificial/Captura%20de%20tela%202026-06-04%20235300.png)

![Página Contatos](https://github.com/soleralucas/assets/blob/main/projeto-inteling%C3%AAncia-artificial/Captura%20de%20tela%202026-06-04%20235157.png)

# Tecnologias utilizadas

* HTML5
* CSS3

# Como executar o projeto

## Pré-requisitos

* Navegador Web (Google Chrome, Microsoft Edge, Firefox, etc.)
* Visual Studio Code (opcional)

## Clonar o repositório

```bash
git clone git@github.com:soleralucas/inteligencia-artificial.git

cd inteligencia-artificial
```

## Executando a aplicação

Abra o arquivo:

```text
index.html
```

em qualquer navegador web.

Caso deseje visualizar ou editar o código-fonte, o projeto também pode ser aberto normalmente no Visual Studio Code e executado utilizando a extensão Live Server.

# Autor

Lucas Pereira Solera

https://www.linkedin.com/in/lucas-pereira-solera/

